...
m_dillon<-brm(TFT~1+Dep+Gram+DepxGram+Int_gram_refl+Int_gram_agr+Int_ungram_refl+Int_ungram_agr + (1+Dep+Gram+DepxGram+Int_gram_refl+Int_gram_agr+Int_ungram_refl+Int_ungram_agr|subj) + (1+Dep+Gram+DepxGram+Int_gram_refl+Int_gram_agr+Int_ungram_refl+Int_ungram_agr|item),crit_orig,family=lognormal(),prior = priors,
...
for(i in 1:40){
 refl_subj[,i]<-exp(m_post[,1]+u0[,i]+(u1_refl[,i]+m_post[,2])/2)-
    exp(m_post[,1]+u0[,i]-(u1_refl[,i]+m_post[,2])/2)
 agr_subj[,i]<-exp(m_post[,1]+u0[,i]+(u1_refl[,i]+m_post[,3])/2)-
    exp(m_post[,1]+u0[,i]-(u1_refl[,i]+m_post[,3])/2)
}
...
for(i in 1:181){
 ...
 agr_subj[,i]<-exp(m_post[,1]+u0[,i]+(u1_agr[,i]+m_post[,3])/2)-
    exp(m_post[,1]+u0[,i]-(u1_agr[,i]+m_post[,3])/2)
}
