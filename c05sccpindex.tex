...
We quantified the goodness of fit between the model predictions and the data using the  \emph{Pearson product-moment correlation coefficient} $R$, and the \index{root-mean-square deviation} \emph{root-mean-square deviation} ($RMSD$).  $RMSDs$ were normalized by the standard deviation of the observed data in the same way as it was done in \citep{Reichle1998} and \cite{Salvucci2001}.  A precise definition is given in the Appendix.
