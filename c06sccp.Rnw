...
Finally, a possible Interface IV: \emph{Subvocalization} is proposed. As another alternative to Time Out, a word ready for integration could be stored in phonological memory \cite{BaddeleyHitch1974,Baddeley2003} for a very short time until there is free capacity of the sentence processor. This could be used in future work to model spill-over effects of parsing difficulty.

Table~\ref{tab:params} summarizes relevant parameter values used in the simulations in this chapter. If not stated otherwise, ACT-R and EMMA parameters were kept constant at the values estimated for model $4$ ``EMMA+r'' in the corpus study of Chapter~\ref{chap:topics} or at default values.


\begin{table}[htp]
  \caption{ACT-R/EMMA parameter values.}
\begin{center}
...
load("data/ch_sims/staub10-original/m.RData")
#head(m)
...
d <- read.table("data/ch_sims/staub10-original/staub10-data.txt", header=TRUE)
...
A likely mechanism for delaying parsing processes in this way is to hold words temporarily in the articulatory loop \cite{BaddeleyHitch1974,Baddeley2003}. This possibility will be discussed and tested with a brief simulation, presented below. 
...
They conducted an eye-tracking experiment using the stimuli of a Spanish study by \cite{MeseguerCarreirasClifton2002} and analyzed the participants' individual scanpaths \cite{MalsburgVasishth2011} and working memory capacity.
...
\begin{xlist}
\item 
\gll \dots cuando los directores \textbf{entraron} en la clase de m{\'u}usica. (HIGH) \\
\dots when the directors entered in the class of music\\
\glt \dots when the directors entered in the class of music.
\item 
\gll cuando los directores \textbf{entraran} en la clase de m{\'u}sica. (LOW)\\
\dots when the directors entered in the class of music\\
\glt \dots when the directors entered in the class of music.
\item 
\gll \dots \textbf{si} los directores entraban en la clase de m{\'u}sica. (UNAMB)\\
...
\begin{figure}[!htbp]
  \centering
%\includegraphics[width=10cm]{figures/malsburgdataambadv}  
%
<<mv13dataambadv,echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=5,fig.height=3, out.width='0.6\\textwidth'>>=
data <- read.table("data/ch_sims/mv13/data-mv13.txt")
...
The evidence summarized above suggests that ambiguous attachment relations are strategically underspecified and that readers with low working memory capacity leave an attachment underspecified more often than do high-capacity readers. 
...
Here, we propose that underspecification results from a single strategy that aims for uninterrupted reading whenever possible.
...
Working memory capacity has been modelled in this way before: \cite{DailyLovettReder2001} modeled individual differences in the digit-span task \cite{LovettRederLebiere1999} in ACT-R by manipulating the source activation $W$ for the goal buffer.  
...
load("data/ch_sims/mv13/sim-etm.RData")
#
##
## ADDITIONAL MEASURES
##
...
##
## encoding times ##
enc <- read.table("data/ch_sims/mv13/enctimes.txt", header=TRUE)
# head(enc,20); dim(enc)
...
## time outs ##
tmo <- read.table("data/ch_sims/mv13/timeouts.txt", header=TRUE)
# head(tmo)

## attachment times ##
retr <- read.table("data/ch_sims/mv13/attachments.txt", header=TRUE)
...
## trial messages
msg <- read.table("data/ch_sims/mv13/trialmessages.txt", header=F)
...
## subject info ##
subjects <- read.table("data/ch_sims/mv13/subjects.txt", header=FALSE)
...
# save(trialinfo, file="mv13/m.trials.RData")
# save(subjects, file="mv13/m.subjects.RData")
...
%
<<mv13modelambadv,echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=5,fig.height=3, out.width='0.6\\textwidth'>>=
...
%\includegraphics[width=10cm]{figures/modeltimeout}  
  %
<<mv13modeltimeout,echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=5,fig.height=3, out.width='0.7\\textwidth'>>=
...
\begin{enumerate}
\item Time Out: Short regressions compensate for slow syntactic integration, simulation on Potsdam Sentence Corpus)
