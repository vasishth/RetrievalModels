...
In this section, we use the statistical modeling approach discussed above to answer a different questiono. Of the two competing theories of retrieval, the Lewis and Vasishth model and the direct-access model, which theory can better explain the empirical facts about Chinese \index{Chinese} \index{relative clause} relative clause processing?
