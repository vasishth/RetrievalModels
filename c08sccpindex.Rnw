...
Theories of sentence processing deficits in aphasia have been traditionally categorized as either \index{representational deficit} representational deficit accounts or \index{processing deficit} processing deficit accounts [for an overview, see][]\citep{Caplan:2009}.
...
\paragraph{Cluster analysis} In order to investigate the
 predicted clustering of parameter estimates, we performed a \index{cluster analysis} cluster analysis on the data too see to which degree controls and IWA could be discriminated.
