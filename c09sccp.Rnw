...
In this closing chapter, we briefly discuss what we see as some of the important open problems that we made some progress in solving, and the problems that still need to be addressed. In Section 1.5, we discussed four gaps in the literature. This book attempts to address each of these gaps.

\section{The relative scarcity of implemented computational models}

In this book, we have presented several models covering a broad range of empirical phenomena.


\section{An excessive focus on average behaviour}

\section{The creation of higher-precision benchmark data-sets for model evaluation and comparison}

Several classes of problems have been studied in sentence processing. A probably incomplete list is shown below:
...
In recent empirical work we have carried out, we have attempted to systematically create benchmark reading data available specifically on similarity-based interference effects. 

\begin{itemize}
\item Mertzen et al 2020a
\item Mertzen et al 2020b
\item \cite{JaegerMertzenVanDykeVasishth2019}
\item \cite{VasishthMertzenJaegerGelman2018}
\item JaegerEngelmannVasishth2017
\end{itemize}

These data are a mixture of complete data-sets, and data summaries derived from published work. It would be useful to systematically create a repository of such data-sets for the different topics listed above, with the goal that the predictions of competing models of sentence processing could be tested against a broad class of phenomena.
