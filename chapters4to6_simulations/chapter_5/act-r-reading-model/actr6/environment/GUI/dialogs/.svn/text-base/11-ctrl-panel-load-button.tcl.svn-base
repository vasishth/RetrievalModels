...
button .control_panel.load -text "Load Model" -font button_font -command {
  global load_return
  ...
  if {$local_connection == 0} {
    ...
    if {$fname != ""} {
      ...
      send_environment_cmd \
        "update [get_handler_name .control_panel.load] \
           (lambda (x) \
              (declare (ignore x)) \
...
pack .control_panel.load

send_environment_cmd \
  "create list-handler .control_panel.load load_return (lambda (x)) ()"

bind .control_panel.load <Destroy> {
  remove_handler %W
}
