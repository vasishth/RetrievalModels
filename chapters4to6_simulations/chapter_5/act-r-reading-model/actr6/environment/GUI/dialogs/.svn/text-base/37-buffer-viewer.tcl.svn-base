...
proc make_buffer_viewer {} {
  ...
  if {[currently_selected_model] == "nil"} {
    ...
    tk_messageBox -icon info -type ok -title "Buffer Viewer" \
  ...
  bind $list_box <<ListboxSelect>> "select_buffer %W $text_box $m"
...
# select_buffer
# Given a list widget (listwin), a tcl window (target_win), and the name
# of the model being used if there is a Lisp handler for target_win and 
# something selected in the listwin send a command to Lisp to request an 
# update of target_win with the result of the buffer_contents command.

proc select_buffer {listwin target_win model_name} {
  ...
  if [valid_handler_name $target_win] {
    ...
    if {[llength $selections] != 0} {
      ...
      send_environment_cmd "update [get_handler_name $target_win] \
            (lambda (x) (declare (ignore x)) (buffer-contents '$buffer))"
      
      if {$options_array(multiple_models) == 1} {
        wm title $top_win "$buffer Buffer viewer ($model_name)"
      } else {
        wm title $top_win "$buffer Buffer viewer"
      }
...
button .control_panel.buffer -command {make_buffer_viewer} \
                    -text "Buffer viewer" -font button_font

# put that button on the control panel

pack .control_panel.buffer
