...
proc make_declarative_history_viewer {} {
  ...
  send_environment_cmd \
      "create list-handler $l1 $win.dummy \
         (lambda (x) (declare (ignore x)) (no-output (sgp :save-dm-history t)) nil) (reset) [send_model_name]"
  ...
  # make chunk selection call the chunk display updater 
  ...
  place $text_frame_1 -relx .45 -y 25 -relheight .7 -height -25 -relwidth .55
  place $l4 -relx .45 -rely .7 -height 25 -relwidth .55
  place $text_frame_2 -relx .45 -rely .7 -y 25 -relheight .3 -height -25 -relwidth .55
...
button .control_panel.declarative_history -command {make_declarative_history_viewer} \
       -text "Retrieval History" -font button_font

# put that button on the control panel

pack .control_panel.declarative_history
