...
;
   ...
   (default-prep-time :accessor default-prep-time :initarg :default-prep-time :initform 0.006) ;; [FE] 
   (prep-fixed :accessor prep-fixed :initarg :prep-fixed :initform T) ;; [FE] 
...
;  (if (eq (exec-s eye-mod) 'BUSY)   ;DEFAULT: attention shift not possible during saccade
;  (if (eq (proc-s eye-mod) 'BUSY)   ;[FE]: attention shift also possible during saccade
...
;;;
      (unless (eq 'BUSY (exec-s eye-mod))  
        (awhen (prep-event eye-mod)
          (no-output (when (car (sgp :v)) (format t "~% +++ !!! Deleting a prep event (SKIP) !!! +++~%")))
               (delete-event it))
...
(defmethod initiate-eye-move ((eye-mod emma-vis-mod) (recog-time number)
  ...
  (let ((sacc-mvmt (build-sacc-mvmt eye-mod r-theta)))
    ...
    (no-output (when (car (sgp :v)) 
        (format t "~%   > Prep-Time: ~6,3F" (fprep-time sacc-mvmt))
        (format t "~%   > Exec-Time: ~6,3F" (exec-time sacc-mvmt))
;        (format t "~%-----------------------------------------------~%")
        ))
...
(defmethod complete-eye-move ((eye-mod emma-vis-mod) (trgt symbol) 
  ...
  ;; [FE] report fixation time
;  (report-fixation-time (write-to-string trgt))
  (report-fixation-time xyloc)
  ...
  (when (and (moving-attention eye-mod)
    ...
    (let* ((start-loc (eye-loc eye-mod))
      ...
      (setf (shift-start eye-mod) (mp-time-ms))
      ;; FE: print out new encoding time
      (no-output (when (car (sgp :v)) (format t "~% +++ Refixation! New encoding time: ~F +++" new-duration)))
...
(defmethod perform-movement ((eye-mod emma-vis-mod) (sacc saccade))
  (if (eq (exec-s eye-mod) 'BUSY)   ;[FE]
    (pm-warning "Eye move could not execute at ~S while one was already in progress."
                (mp-time))
    (progn
...
(defmethod recog-time ((eye-mod emma-vis-mod) obj (r-theta vector))
  (let ( 
        (freq (object-frequency eye-mod obj (default-freq eye-mod)))
        ;(freq (frequency obj))   ;[FE]
...
;; modified encoding time equation
;    (setf enctime
;          (rand-time (* (enc-factor eye-mod)
;                        (+
;                         (- (log freq))
...
;                        (exp (* eccentricity (enc-exponent eye-mod))))))
    ...
    (setf enctime
          (rand-time (*  (+
                            (* (enc-factor eye-mod) (- (log freq)))
                            surpr
                          )
                          (exp (* eccentricity (enc-exponent eye-mod))))))

    (report-attention-shift obj freq enctime eccentricity surpr)   ; [FE]
    (output-enctime obj enctime)
...
;; [FE] dummy functions
(defun output-enctime (obj enctime) nil)
(defun my-frequency (obj) nil)
(defun report-fixation-time (loc) nil)
(defun surprisal (obj) 0)
  ...
  (let ((value (fast-chunk-slot-value-fct obj 'value)))
    (if (stringp value)
        (aif (gethash value (freq-ht eye-mod))
             it
             (aif (my-frequency obj) it default))   ;[FE] user-defined frequency function
             ;default)
...
; [FE] allow to choose if prept-time is calculated or set manually
;    (if (eq *my-prep-time* nil) ;[FE]
    (if (prep-fixed eye-mod) ;[FE]
;      (setf (fprep-time sacc-mvmt) (rand-time *my-prep-time*)))   ;[FE]
      (setf (fprep-time sacc-mvmt) (rand-time (default-prep-time eye-mod)))   ;[FE]
      (setf (fprep-time sacc-mvmt) (rand-time (compute-prep-time eye-mod sacc-mvmt)))
      )
 ...
 (aif (params-vision-module vis-mod param)
      ...
      (if (consp param)
        (case (car param)
          ...
          (:SACCADE-PREPARATION-TIME   ; [FE]
           (setf (default-prep-time vis-mod) (cdr param)))
          (:FIXED-PREP-TIME   ; [FE]
           (setf (prep-fixed vis-mod) (cdr param)))
          )
        (case param
          (:VISUAL-ENCODING-FACTOR
           (enc-factor vis-mod) )
...
(define-module-fct :vision 
  ...
  (list 
   ...
   (define-parameter :vis-obj-freq
                      ...
                      :warning "a non-negative number" 
                      :default-value 0.01
                      :documentation "Default visual object frequency for EMMA")
;;; [FE] new fixed preparation time
   (define-parameter :SACCADE-PREPARATION-TIME
                      :valid-test #'nonneg
                      :warning "a non-negative number" 
                      :default-value 0.006
                      :documentation "Default visual preparation time for EMMA")
   (define-parameter :FIXED-PREP-TIME   ;; [FE]
                      :valid-test #'tornil
                      :warning "T or NIL" 
                      :default-value T
                      :documentation "Whether the fixed default prep-time should be used or a dynamic calculation")
   )
  :warning 'warn-vision
  :version "4.0a1-emma"
