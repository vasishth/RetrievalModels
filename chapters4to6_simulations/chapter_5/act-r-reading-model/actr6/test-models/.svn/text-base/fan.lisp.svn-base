...
(defun output-person-location (data)
  (let ((rts (mapcar 'first data)))
    (correlation rts *person-location-data* :output *out-file*)
    (mean-deviation rts *person-location-data* :output *out-file*)
    (model-output "~%TARGETS:~%                         Person fan~%")
    (model-output  "  Location      1             2             3~%")
    (model-output "    fan")
    
    (dotimes (i 3)
      (model-output "~%     ~d    " (1+ i))
      (dotimes (j 3)
        (model-output "~{~8,3F (~3s)~}" (nth (+ j (* i 3)) data))))
    
    (model-output "~%~%FOILS:")
    (dotimes (i 3)
      (model-output "~%     ~d    " (1+ i))
      (dotimes (j 3)
        (model-output "~{~8,3F (~3s)~}" (nth (+ j (* (+ i 3) 3)) data))))))

(defvar *act-flag* nil)
(defvar *break-run* nil)

(defun run-it ()
  
  (setf *act-flag* nil)
  (test-sentence-model "lawyer" "store" t 'person)
  (sdm isa comprehend-sentence)
  (setf *break-run* t)
  (test-sentence-model "lawyer" "store" t 'person)
  ...
  (test-sentence-model "lawyer" "store" t 'person)
  (test-sentence-model "hippie" "bank" t 'person)
  (test-sentence-model "hippie" "bank" t 'location)
  (test-sentence-model "giant" "bank" t 'person)
  (test-sentence-model "giant" "bank" t 'location)
  (setf *act-flag* nil)
...
(sgp-fct (list :v *out-file* :cmdt *out-file* :act *act-flag*))
(sgp :seed (1 0))
...
(when *break-run* (pbreak encode-person))
