...
(defun display-results (results)
  (let ((questions '("1+1" "1+2" "1+3" "2+2" "2+3" "3+3")))
    (correlation results *siegler-data* :output *out-file*)
    (mean-deviation results *siegler-data* :output *out-file*)
    (model-output "       0     1     2     3     4     5     6     7     8   Other~%")
    (dotimes (i 6)
      (model-output "~a~{~6,2f~}~%" (nth i questions) (nth i results)))))
