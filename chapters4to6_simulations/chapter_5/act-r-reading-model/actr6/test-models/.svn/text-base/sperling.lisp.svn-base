...
(defvar *a-trace-level* 'low)

(defun run-it ()
  (setf *a-trace-level* 'low)
  (do-trial .15)
  (setf *a-trace-level* 'high)
  (do-trial .15))
...
(sgp :show-focus t :needs-mouse nil)

(sgp :seed (100 0))

(sgp-fct (list :v *out-file* :cmdt *out-file* :trace-detail *a-trace-level*))
